-  # 面试官：说说微信小程序的实现原理？
   ...
   ![](https://www.oss.tuwei.site/blogsImgs/fe/4407cb60-3722-11ec-a752-75723a64e8f5.png)
  ...
  而在小程序中，选择了 `Hybrid` 的渲染方式，将视图层和逻辑层是分开的，双线程同时运行，视图层的界面使用 `WebView` 进行渲染，逻辑层运行在 `JSCore` 中
   ...
   ![](https://www.oss.tuwei.site/blogsImgs/fe/4e322e50-3722-11ec-8e64-91fdec0f05a1.png)
  ...
  在逻辑层发生数据变更的时候，通过宿主环境提供的`setData`方法把数据从逻辑层传递到渲染层，再经过对比前后差异，把差异应用在原来的`Dom`树上，渲染出正确的视图
   ...
   ![](https://www.oss.tuwei.site/blogsImgs/fe/5948ed10-3722-11ec-a752-75723a64e8f5.png)

  当视图存在交互的时候，例如用户点击你界面上某个按钮，这类反馈应该通知给开发者的逻辑层，需要将对应的处理状态呈现给用户

  对于事件的分发处理，微信进行了特殊的处理，将所有的事件拦截后，丢到逻辑层交给`JavaScript`进行处理

   ![](https://www.oss.tuwei.site/blogsImgs/fe/61f9f670-3722-11ec-a752-75723a64e8f5.png)
  ...
  因此逻辑层与渲染层需要有一定的机制保证时序正确，在每个小程序页面的生命周期中，存在着若干次页面数据通信
   ...
   ![](https://www.oss.tuwei.site/blogsImgs/fe/6cb798b0-3722-11ec-a752-75723a64e8f5.png)
  ...
  >  3.短时间内收到系统两次以上内存警告，也会对小程序进行销毁，这也就为什么一旦页面内存溢出，页面会奔溃的本质原因了
   ...
   ![](https://www.oss.tuwei.site/blogsImgs/fe/968c8510-3722-11ec-a752-75723a64e8f5.png)
